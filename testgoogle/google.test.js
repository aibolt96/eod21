...
test("can search Google twice", async () => {
  ...
  await driver.findElement(By.name('q')).sendKeys("Marvel Snap", Key.RETURN);
  ...
  await driver.findElement(By.name('q')).sendKeys("DC dual force", Key.RETURN);
  // TODO Wait for the results page to load
  await driver.wait(until.titleIs("DC dual force - Google Search"), 1000);
});
